...
bool VertexClassifier::isLeft(double x0, double y0, double x1, double y1, double x2, double y2){
...
std::vector<Segment2D> VertexClassifier::extractSegments(const MeshGraph& graph, double height) {
    ...
    for (int fidx = 0; fidx < graph.faces.size(); ++fidx) {
        const auto& f = graph.faces[fidx];
        ...
        if (onCount == 3) continue;

        auto addSeg = [&](double x0, double y0, double x1, double y1, bool checkLeft, double x2, double y2) {
            if (checkLeft) {
                if (isLeft(x0, y0, x1, y1, x2, y2))
                    rawSegments.emplace_back(x0, y0, x1, y1, SegmentType::INTERIOR, fidx);
                else
                    rawSegments.emplace_back(x1, y1, x0, y0, SegmentType::INTERIOR, fidx);
            } else {
                if (!isLeft(x0, y0, x1, y1, x2, y2))
                    rawSegments.emplace_back(x0, y0, x1, y1, SegmentType::INTERIOR, fidx);
                else
        ...
        if (belowCount == 1 && aboveCount == 2) {
            ...
            int e0 = (aboveIdx + 1) % 3;
            int e1 = (aboveIdx + 2) % 3;
            auto [x0, y0] = interpolate(*v[aboveIdx], *v[e0], height);
            auto [x1, y1] = interpolate(*v[aboveIdx], *v[e1], height);
            double x2 = v[e0]->x;
            double y2 = v[e0]->y;
            addSeg(x0, y0, x1, y1, true, x2, y2);
        }
    ...
    for (size_t fidx = 0; fidx < graph.faces.size(); ++fidx) {
        const auto& f = graph.faces[fidx];
        edgeToFaces[{f.v0, f.v1}].push_back(static_cast<int>(fidx));
        edgeToFaces[{f.v1, f.v2}].push_back(static_cast<int>(fidx));
        edgeToFaces[{f.v2, f.v0}].push_back(static_cast<int>(fidx));
    }
    ...
    for (const auto& seg : rawSegments) {
        ...
        for (int faceIdx : it->second) {
            ...
            VertexState effectiveState = (st == VertexState::ON) ? VertexState::ABOVE : st;

            if (effectiveState != VertexState::BELOW) allBelow = false;
            if (effectiveState != VertexState::ABOVE) allAbove = false;
        }
