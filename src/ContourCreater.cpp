...
bool ContourCreater::isPointOnSegment(double px, double py, double ax, double ay, double bx, double by) {
    ...
    if (std::fabs(cross) > 1e-6) return false;
...
void ContourCreater::addBoundarySegments(const ParsedMesh& mesh, const std::vector<Segment2D>& interiorSegments, std::vector<Segment2D>& allSegments) {
    for (size_t p = 0; p < mesh.polygons.size(); ++p) {
        ...
        if ((isOuter && area < 0) || (!isOuter && area > 0)) {
            std::reverse(poly.vertexIDX.begin(), poly.vertexIDX.end());
            area = -area;
        }
        ...
        for (size_t i = 0; i < poly.vertexIDX.size(); ++i) {
            ...
            std::vector<std::pair<double, std::pair<double, double>>> edgePoints;
            edgePoints.emplace_back(0.0, std::make_pair(pu.x, pu.y));
            edgePoints.emplace_back(1.0, std::make_pair(pv.x, pv.y));
            ...
            std::vector<std::pair<double, std::pair<double, double>>> filtered;
            for (size_t k = 0; k < edgePoints.size(); ++k) {
                if (k == 0 || edgePoints[k].first - filtered.back().first > 1e-6) {
                    filtered.push_back(edgePoints[k]);
                }
            }

            for (size_t k = 0; k + 1 < filtered.size(); ++k) {
                double xx0 = filtered[k].second.first, yy0 = filtered[k].second.second;
                double xx1 = filtered[k+1].second.first, yy1 = filtered[k+1].second.second;
                if (std::hypot(xx1 - xx0, yy1 - yy0) > 1e-9) {
                    allSegments.emplace_back(xx0, yy0, xx1, yy1, SegmentType::BOUNDARY, -1);
                }
            }
        }
    }
...
std::vector<std::vector<Point2D>> ContourCreater::create(const std::vector<Segment2D>& segments, const ParsedMesh& mesh) {

    std::vector<Segment2D> allSegments = segments;
    addBoundarySegments(mesh, segments, allSegments);

    if (allSegments.empty()) return {};

    std::unordered_map<PointKey, Point2D, PointKeyHash> pointMap;
    ...
    std::unordered_map<PointKey, int, PointKeyHash> pointToIndex;
    ...
    std::vector<DirectedSegment> dirSegs;
    ...
    std::vector<std::vector<int>> outEdges(pts.size());
